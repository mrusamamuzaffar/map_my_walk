...
import 'package:map_my_walk/models/stats.dart';
      ...
      data.listen(
        (event) {
          ...
          final List<Stats> auctions = List.generate(raw.length, (index) {
            totalSteps += (raw[index].data() as dynamic)["userStepCount"] as int;
            totalCaloriesBurned += (raw[index].data() as dynamic)["caloriesBurned"] as int;
            averageSpeed += (raw[index].data() as dynamic)["speed"] as double;
            totalDistanceCovered += (raw[index].data() as dynamic)["distanceTraveled"] as double;
